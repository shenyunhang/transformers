...
class YoutuVITAOmniRMSNorm(Qwen3RMSNorm):
    pass
...
class YoutuVITAOmniChunkedMTHWRotaryEmbedding(nn.Module):
    ...
    def forward(self, pos_ids: torch.Tensor) -> torch.Tensor:
        if pos_ids.dim() != 2 or pos_ids.size(-1) != 4:
            raise ValueError(
                f"YoutuVITAOmniChunkedMTHWRotaryEmbedding expects pos_ids of "
...
class YoutuVITAOmniMLP(Qwen3MLP):
    """SwiGLU MLP (Qwen3 style). Inherits :class:`YoutuMLP` directly."""

    pass


class YoutuVITAOmniFlashAttention2(Qwen3Attention):
