...
class YoutuVITAOmniFourDRotaryEmbedding(nn.Module):
    ...
    def forward(self, pos_ids: torch.Tensor) -> torch.Tensor:
        if pos_ids.dim() != 2 or pos_ids.size(-1) != 4:
            raise ValueError(
                f"YoutuVITAOmniFourDRotaryEmbedding expects pos_ids of shape [S, 4], got {tuple(pos_ids.shape)}"
            )
...
class YoutuVITAOmniEncoder(YoutuVITAOmniPreTrainedModel):
    ...
    def vision_rot_pos_emb_4d(
        ...
        if self.rotary_pos_emb_4d is None:
            raise RuntimeError("vision_rot_pos_emb_4d called but use_4d_rope is False")
        pos_ids_list = []
    ...
    def audio_rot_pos_emb_4d(
        ...
        """
        if self.rotary_pos_emb_4d is None:
            raise RuntimeError("audio_rot_pos_emb_4d called but use_4d_rope is False")
        pos_ids_list = []
        for length in lens.tolist():
            n = int(length)
            if n <= 0:
