...
class Mamba3Qwen3VITAVisionPatchMerger(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # print(f"VisionPatchMerger input {x.shape=} {x.max()=} {x.min()=} {x.abs().mean()=}")
        x = self.norm(x.reshape(-1, self.hidden_size))
        # print(f"VisionPatchMerger after norm {x.shape=} {x.max()=} {x.min()=} {x.abs().mean()=}")
        x = self.linear_fc2(self.act_fn(self.linear_fc1(x)))
        # print(f"VisionPatchMerger output {x.shape=} {x.max()=} {x.min()=} {x.abs().mean()=}")
        return x
...
class Mamba3Qwen3VITAVisionEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        """
        # print(f"Mamba3VisionEncoderLayer input {self.layer_idx=} {self.layer_type=} {hidden_states.shape=} {hidden_states.max()=} {hidden_states.min()=} {hidden_states.abs().mean()=}")
        if self.layer_type == "M":
            residual = hidden_states
            hidden_states = self.input_layernorm(hidden_states)
            # print(f"Mamba3VisionEncoderLayer after norm {self.layer_idx=} {self.layer_type=} {hidden_states.shape=} {hidden_states.max()=} {hidden_states.min()=} {hidden_states.abs().mean()=}")
            ...
            # print(f"Mamba3VisionEncoderLayer after mixer {self.layer_idx=} {self.layer_type=} {hidden_states.shape=} {hidden_states.max()=} {hidden_states.min()=} {hidden_states.abs().mean()=}")
            ...
            # print(f"Mamba3VisionEncoderLayer after mlp {self.layer_idx=} {self.layer_type=} {hidden_states.shape=} {hidden_states.max()=} {hidden_states.min()=} {hidden_states.abs().mean()=}")
            hidden_states = residual + hidden_states

        else:
            raise ValueError(f"Invalid layer type: {self.layer_type}")

        # print(f"Mamba3VisionEncoderLayer output {self.layer_idx=} {self.layer_type=} {hidden_states.shape=} {hidden_states.max()=} {hidden_states.min()=} {hidden_states.abs().mean()=}")
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if fake_images is not None:
            ...
            # print(f"inputs_embeds without image_embeds {inputs_embeds.shape=} {inputs_embeds.max()=} {inputs_embeds.min()=} {inputs_embeds.abs().mean()=}")
