...
class Mamba3Qwen3VITATextRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps: float = 1e-6) -> None:
        """
        Mamba3Qwen3VITATextRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class Mamba3Qwen3VITATextRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: Mamba3Qwen3VITATextConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: Mamba3Qwen3VITATextConfig | None = None,
...
class Mamba3Qwen3VITATextMLP(nn.Module):
...
class Mamba3Qwen3VITATextAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: Mamba3Qwen3VITATextConfig, layer_idx: int):
        super().__init__()
        ...
        self.q_norm = Mamba3Qwen3VITATextRMSNorm(
            self.head_dim, eps=config.rms_norm_eps
        )  # unlike olmo, only on the head dim!
        self.k_norm = Mamba3Qwen3VITATextRMSNorm(
            self.head_dim, eps=config.rms_norm_eps
...
class Mamba3Qwen3VITATextDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Mamba3Qwen3VITATextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = Mamba3Qwen3VITATextAttention(config=config, layer_idx=layer_idx)

        self.mlp = Mamba3Qwen3VITATextMLP(config)
        self.input_layernorm = Mamba3Qwen3VITATextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Mamba3Qwen3VITATextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class Mamba3Qwen3VITAVisionEncoder(nn.Module):
    ...
    def __init__(self, config: Mamba3Qwen3VITAVisionConfig):
        ...
        self.rotary_pos_emb = Mamba3Qwen3VITAVisionRotaryEmbedding(
...
@auto_docstring
class Mamba3Qwen3VITATextModel(Mamba3Qwen3VITAPreTrainedModel):
    config: Mamba3Qwen3VITATextConfig

    def __init__(self, config: Mamba3Qwen3VITATextConfig):
        ...
        self.layers = nn.ModuleList(
            [Mamba3Qwen3VITATextDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = Mamba3Qwen3VITATextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Mamba3Qwen3VITATextRotaryEmbedding(config=config)
        self.gradient_checkpointing = False
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # print(f"Mamba3Qwen3VITATextModel input {hidden_states.shape=} {hidden_states.abs().max()=} {hidden_states.abs().mean()=}")
        for layer_idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            # print(f"Mamba3Qwen3VITATextModel {layer_idx=} {hidden_states.shape=} {hidden_states.abs().max()=} {hidden_states.abs().mean()=}")
...
__all__ = ["Mamba3Qwen3VITAPreTrainedModel", "Mamba3Qwen3VITAModel", "Mamba3Qwen3VITAForCausalLM"]
